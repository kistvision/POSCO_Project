...
class TrainValidImageDataset(Dataset):
    ...
    def __getitem__(self, batch_index: int) -> [dict[str, Tensor], dict[str, Tensor]]:
        ...
        if self.mode == "Train":
            ...
            # gt_crop_image  = imgproc.random_rotate(gt_crop_image, angles=[0, 90, 180, 270])
            # gt_crop_image= imgproc.random_horizontally_flip(gt_crop_image, p=0.5)
            # gt_crop_image= imgproc.random_vertically_flip(gt_crop_image, p=0.5)
        ...
        # gt_crop_image = imgproc.random_gaussianblur(gt_crop_image, p=1)        

        # print("before ",gt_crop_image.shape)
        # gt_crop_image = imgproc.image_resize(gt_crop_image, 1 / (self.upscale_factor))
        # gt_crop_image = imgproc.image_resize(gt_crop_image, (self.upscale_factor))

        # lr_crop_image = imgproc.image_resize(gt_crop_image, 1 / (self.upscale_factor))

        # lr_crop_image = imgproc.image_resize(gt_crop_image, 1/4)
        # print("bef ",lr_crop_image.size)
        # lr_crop_image = imgproc.image_resize(lr_crop_image, 2)
        # print("aft ",lr_crop_image.size())
        # lr_crop_image = imgproc.random_gaussianblur(lr_crop_image, p=0.5)        

        height, width = gt_crop_image.shape[:2]

        # resized_image = cv2.resize(gt_crop_image, (width//4, height//4), interpolation=cv2.INTER_CUBIC)
        # lr_crop_image = cv2.resize(resized_image, (width//2, height//2), interpolation=cv2.INTER_CUBIC)



        lr_crop_image = imgproc.image_resize(gt_crop_image, 1 / (self.upscale_factor))
        
        # print("after ", lr_crop_image.shape)
        # 화질을 더 낮춰서 학습
        # lr_crop_image = cv2.GaussianBlur(lr_crop_image,(0,0),1)


        # lr_crop_image, gt_crop_image = imgproc.random_rotate2(lr_crop_image, gt_crop_image, angles=[0, 90, 180, 270])
        # lr_crop_image, gt_crop_image = imgproc.random_horizontally_flip2(lr_crop_image, gt_crop_image, p=0.5)
        # lr_crop_image, gt_crop_image = imgproc.random_vertically_flip2(lr_crop_image, gt_crop_image, p=0.5)
        lr_crop_image = imgproc.random_gaussianblur(lr_crop_image, p=0.8)        
        # lr_crop_image = imgproc.random_resize(lr_crop_image, p=0.9)        
...
class TestImageDataset(Dataset):
    ...
    def __getitem__(self, batch_index: int) -> [torch.Tensor, torch.Tensor]:
        ...
        # gt_image = imgproc.random_gaussianblur(gt_image, p=1)        
        # lr_image = imgproc.random_gaussianblur(lr_image, p=1)        

        # 화질을 더 낮춰서 학습
        # lr_image = imgproc.random_gaussianblur(lr_image, p=1)        
        # lr_image = cv2.GaussianBlur(lr_image,(0,0),1)
        # gt_image = imgproc.image_resize(gt_image,1/2)
        # gt_image = imgproc.image_resize(gt_image,2)

        # lr_image = imgproc.image_resize(lr_image,1/2)
        # lr_image = imgproc.image_resize(lr_image,2)
